...
{	
	...
	{	
		cerr << "Error: Filter length is greater than signal length" << endl;
		exit(1);
	}
	...
	for (int i = 0; i < exLen; i++)
	{
		...
		for (int j = 0; j < m_dbFilter.filterLen; j++)
		{
			...
			if ((k<0) && (k >= -m_dbFilter.filterLen + 1))
				...
				tmp = pSrcData[k];
			else if ((k>srcLen - 1) && (k <= (srcLen + m_dbFilter.filterLen - 2)))
	...
	)
{
	...
	{
		cerr << "Error: DWT2 signal pointer is null" << endl;
		// Sleep(3000);
		exit(1);
	}
	...
	)
{
	...
	{
		cerr << "Error: Filter length is greater than image height / width" << endl;
		return false;
	}
	...
	for (int i = 1; i <= Scale; i++)
	{
		int exHeight = (srcHeight + m_dbFilter.filterLen - 1) / 2;
		srcHeight = exHeight;
		m_msgCL2D.msgHeight[i] = srcHeight;

		int exWidth = (srcWidth + m_dbFilter.filterLen - 1) / 2;
		srcWidth = exWidth;
	...
	bool isHard)
{
	...
	int gapLL = m_msgCL2D.msgHeight[m_msgCL2D.Scale + 1] * m_msgCL2D.msgWidth[m_msgCL2D.Scale + 1];
